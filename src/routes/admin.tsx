...
import {
  Users, Search, CreditCard, CheckCircle, XCircle, Clock,
  ArrowUpRight, LogOut, TrendingUp, MapPin, Hash, Eye,
  ChevronLeft,
} from "lucide-react";
...
type ParticipantRow = {
  ...
  payments?: { id: string; amount: number; payment_method: string; payment_status: string; created_at: string; receipt_url: string | null }[];
};
...
function AdminPage() {
  ...
  const [stats, setStats] = useState<Stats>({ total: 0, approved: 0, pending: 0, review: 0, capacity_left: 2000 });
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: payments, error: _pye } = await supabase
      .from("payments")
      .select("id, participant_id, amount, payment_method, payment_status, created_at, receipt_url");
  ...
  const handleApprove = async (paymentId: string) => {
    const { error } = await supabase
      ...
      .eq("id", paymentId);
    if (!error) { setActionMsg("Pago aprobado."); fetchData(); }
  ...
  const handleReject = async (paymentId: string) => {
    const { error } = await supabase
      ...
      .eq("id", paymentId);
    if (!error) { setActionMsg("Pago rechazado."); fetchData(); }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-run-beige px-5">
        <div className="w-full max-w-sm">
          <a href="/" className="mb-6 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-run-orange hover:opacity-80">
            <ChevronLeft className="h-3.5 w-3.5" /> Volver
          </a>
          <h1 className="editorial-display text-4xl text-run-blue">Admin</h1>
          <p className="mt-2 text-sm text-muted-foreground">Inicia sesión para gestionar el evento.</p>
          <form
          ...
          <p className="mt-6 text-center text-xs text-muted-foreground">
            Usa las credenciales configuradas en Supabase Auth.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-run-beige">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:py-12">
        <div className="flex items-center justify-between">
          ...
          <button onClick={handleLogout} className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-muted-foreground hover:text-run-orange">
        ...
        {actionMsg && (
          <div className="mt-4 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
            {actionMsg}
            <button onClick={() => setActionMsg(null)} className="ml-3 font-bold">&times;</button>
          </div>
        )}
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            { label: "Pendientes", value: stats.pending + stats.review, icon: Clock, color: "text-amber-600" },
            { label: "Lugares libres", value: stats.capacity_left, icon: TrendingUp, color: stats.capacity_left > 100 ? "text-green-600" : "text-red-600" },
          ].map((s) => (
            <div key={s.label} className="rounded-2xl border border-border bg-white p-5">
              ...
              <p className={`mt-2 font-display text-3xl font-bold ${s.color}`}>{s.value.toLocaleString("es-MX")}</p>
            </div>
          ))}
        </div>
            ...
            {(["all", "pending", "review"] as const).map((t) => (
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-xs font-semibold uppercase tracking-wider transition ${
                  tab === t ? "bg-run-orange text-white" : "border border-border bg-white text-muted-foreground hover:border-run-orange/50"
                }`}
              >
        ...
        <div className="mt-4 overflow-x-auto rounded-2xl border border-border bg-white">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="px-5 py-12 text-center text-muted-foreground">Cargando…</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={6} className="px-5 py-12 text-center text-muted-foreground">Sin resultados.</td></tr>
              ) : (
                filtered.slice(0, 100).map((p) => {
                  const latestPayment = p.payments?.[0];
                  return (
                    <tr key={p.id} className="border-b border-border/50 last:border-0 hover:bg-run-beige/50">
                      <td className="px-5 py-4">
                        <p className="font-semibold text-run-blue">{p.first_name} {p.last_name}</p>
                        {p.is_minor && <span className="text-[10px] font-semibold uppercase text-amber-600">Menor</span>}
                      </td>
                      <td className="px-5 py-4 text-xs text-muted-foreground">{p.email}</td>
                      <td className="px-5 py-4 text-xs">{p.race_categories?.distance_km ?? "—"}K</td>
                      <td className="px-5 py-4 font-mono text-xs font-semibold">{p.registration_folio ?? "—"}</td>
                      <td className="px-5 py-4">
                        {latestPayment ? (
                          <span className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${
                            latestPayment.payment_status === "approved" ? "bg-green-50 text-green-700 border border-green-200" :
                            latestPayment.payment_status === "review" ? "bg-blue-50 text-blue-700 border border-blue-200" :
                            latestPayment.payment_status === "rejected" ? "bg-red-50 text-red-700 border border-red-200" :
                            "bg-amber-50 text-amber-700 border border-amber-200"
                          }`}>
                            {latestPayment.payment_method === "mercadopago" ? "MP" : "TRANSF"} · {latestPayment.payment_status}
                          </span>
                        ) : (
