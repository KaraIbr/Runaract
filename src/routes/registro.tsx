...
export const Route = createFileRoute("/registro")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Inscríbete a Runnaract 2.0 powered by Zitara. 3K · 5K · 10K en Zitara Golf Club." },
    ],
  }),
...
const formSchema = z.object({
  ...
  gender: z.enum(["male", "female", "prefer_not_to_say"], { required_error: "Selecciona un género" }),
  distance: z.enum(["3k", "5k", "10k"], { required_error: "Selecciona una distancia" }),
  phone: z.string().optional(),
  emergency_contact_name: z.string().optional(),
  emergency_contact_phone: z.string().optional(),
  terms_accepted: z.literal(true, { errorMap: () => ({ message: "Debes aceptar términos y condiciones" }) }),
  privacy_accepted: z.literal(true, { errorMap: () => ({ message: "Debes aceptar el aviso de privacidad" }) }),
  consent_accepted: z.literal(true, { errorMap: () => ({ message: "Debes aceptar el consentimiento informado" }) }),
...
function getCategoryId(distance: string, gender: string, categories: Array<{ id: number; name: string; distance_km: number }>): number {
  const categoryGender = gender === "male" ? "Varonil" : "Femenil";
  const km = DISTANCES.find((d) => d.value === distance)?.km ?? 3;
  const match = categories.find(
    (c) => c.name === categoryGender && c.distance_km === km,
  );
  return match?.id ?? 1;
}
...
function RegistroPage() {
  ...
  const [categories, setCategories] = useState<Array<{ id: number; name: string; distance_km: number }>>([]);
  ...
  const isMinor = birthYear && birthMonth
    ? calculateAge(Number(birthYear), Number(birthMonth)) < 18
    : false;
  ...
  const selectedPrice = distance ? prices[distance] : null;
      ...
      <div className="mx-auto max-w-2xl px-5 py-12 sm:py-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="editorial-display mt-3 text-5xl text-run-blue sm:text-6xl">
            Tu lugar<br />te espera.
          </h1>
          <p className="mt-4 max-w-md text-muted-foreground">
            Completa tus datos y asegura tu lugar en Runnaract 2.0 —{" "}
            {EVENT.venue}. {EVENT.date.toLocaleDateString("es-MX", { day: "numeric", month: "long", year: "numeric" })}.
          </p>
        </motion.div>
                      ...
                      <RadioGroup onValueChange={field.onChange} value={field.value} className="flex flex-wrap gap-4">
                        {GENDERS.map((g) => (
                          <div key={g.value} className="flex items-center gap-2">
                            ...
                            <Label htmlFor={`gender-${g.value}`} className="cursor-pointer font-normal">{g.label}</Label>
                          </div>
                        ))}
                      </RadioGroup>
                    </FormControl>
                      ...
                      <RadioGroup onValueChange={field.onChange} value={field.value} className="flex flex-wrap gap-4">
                        {DISTANCES.map((d) => (
                          <div key={d.value} className="flex items-center gap-2">
                            ...
                            <Label htmlFor={`dist-${d.value}`} className="cursor-pointer font-normal">
                      ...
                      <div className="text-sm font-normal leading-snug pt-0.5">
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <button type="button" className="text-run-orange underline underline-offset-2 hover:brightness-110 cursor-pointer">
                    ...
                    <FormItem className="flex items-start gap-3 space-y-0">
                      ...
                      <div className="text-sm font-normal leading-snug pt-0.5">
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="max-h-[80vh] max-w-2xl overflow-y-auto">
                            <DialogHeader>
                              <DialogTitle>Consentimiento Informado y Exoneración de Responsabilidad</DialogTitle>
                            </DialogHeader>
                            <div className="whitespace-pre-line text-sm leading-relaxed text-foreground">
                              {INFORMED_CONSENT}
                            </div>
                ...
                {isMinor && (
                  <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50/50 p-5 space-y-4">
                    ...
                    <p className="text-xs text-amber-700">
                      Al ser menor de edad, se requiere una carta responsiva firmada por tu padre, madre o tutor legal.
                      Descarga e imprime el documento, fírmalo junto con tu tutor y súbelo en PDF.
                    </p>
                    <Dialog>
                      <DialogTrigger asChild>
                        <button type="button" className="text-xs font-medium text-run-orange underline underline-offset-2 hover:brightness-110 cursor-pointer">
